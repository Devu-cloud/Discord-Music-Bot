...
class Events(commands.Cog):
    ...
    #part1
    #when member joins discord server
    @commands.Cog.listener()
    async def on_member_join(self,member):
        try:
            ...
            channel_id = 1216737091082256384
            channel = guild.get_channel(channel_id)
            
            embed = discord.Embed(
                title =f"Greetings:",
                description=f"welcome to the server: {guildname} , {member.mention} Enjoy your stay and feel free to look around :)",
                color =0x00FFFF
            )
    ...
    #part2
    #when member leaves the discord server    
    @commands.Cog.listener()
    async def on_member_remove(self,member):
        try:
            ...
            guildname = guild.name
            dmchannel = await member.create_dm()
            channel_id = 1216966481494147184
            channel = guild.get_channel(channel_id)
    ...
    async def on_voice_state_update(self, member, before, after):
        """
        Tracks user voice channel activity and logs it to a designated channel.
        ...
        # Get the logging channel (replace with your actual channel ID)
        log_channel = self.bot.get_channel(1217349679138865164)
        if not log_channel:
            print(f"Logging channel not found: {1217349679138865164}")
            return
